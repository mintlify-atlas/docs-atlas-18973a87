...
description: "Learn how advanced-mark.js achieves fast highlighting and how to optimize for large documents using the CSS Highlight API, combineBy, and other techniques."
---
...
The `combineBy` option (formerly `combinePatterns`) controls how many individual terms from a search array are combined into a single regular expression pattern per run. The default is `10`, meaning an array of 50 terms produces 5 combined patterns and 5 matching passes:

```js
instance.mark(['str1', 'str2', /* ... */], {
  combineBy: 10 // default
});
```
